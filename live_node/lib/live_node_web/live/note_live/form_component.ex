defmodule LiveNodeWeb.NoteLive.FormComponent do
  ...
  def update(%{note: note} = assigns, socket) do
    changeset = Notetaking.change_note(note)
  ...
  def handle_event("validate", %{"note" => note_params}, socket) do
    changeset =
      socket.assigns.note
      |> Notetaking.change_note(note_params)
  ...
  defp save_note(socket, :edit, note_params) do
    case Notetaking.update_note(socket.assigns.note, note_params) do
      {:ok, note} ->
  ...
  defp save_note(socket, :new, note_params) do
    case Notetaking.create_note(note_params) do
      {:ok, note} ->
        notify_parent({:saved, note})
